...
def get_nns(x_o, other_data, other_labels, model):
    ...
    for j in range(len(other_data)):
        usecross=0
        if usecross:
            distances[j] = cross_entrophy(x_preds, other_data_preds[j])
        else:
...
def find_x_i(model, x, y, x_train, y_train):
    ...
    if os.path.exists(args.x_i_information):
        ...
        print('xiidx:',x_i_idx[:20], 'length:', len(x_i_idx))
        ...
        matrix_x_i = np.hstack((np.array(x_i_idx).reshape(len(x_i_idx), 1),
                                ...
                                np.array(model.predict(x_i)).reshape(len(x_i_idx), 10)
                                         ))
        ...
        print('xiidx　without own class:',x_i_idx_wo_own[:20],'length:', len(x_i_idx_wo_own))
        ...
        if x_i_from_target_train:
            ...
            for i in x_i_idx_wo_own:
                if y_train[i] == args.target_class:
                    x_i_idx_wo_own_target.append(i)

            x_i_idx = x_i_idx_wo_own_target
            x_i = x_train[x_i_idx]
            print('xiidx　without own&target class:', x_i_idx[:20], 'length:', len(x_i_idx))
...
def main():
    ...
    for idx in index[:10]:
        ...
        x = x_test[idx]
        print('preded labels of x:', y_test[idx])
        print('preds of x:', model.predict(x))
        ...
        if args.slt_x_i:
            find_x_i(model, x, y_test[idx], x_train, y_train)
